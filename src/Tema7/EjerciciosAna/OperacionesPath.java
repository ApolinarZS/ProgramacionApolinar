...
public class OperacionesPath {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Para obtener un objeto Path utilizamos el mét Path.of() o Paths.get()
        Path unPath = Path.of("carpetaPrueba");
        Path otroPath = Paths.get("NuevaCarpeta");

        // Obtener el nombre de un fichero y unirlo a la ruta de otro directorio
        Path unFile = Path.of("carpetaPrueba/Ejemplo");
        Path unDir = Path.of("NuevaCarpeta/Ejemplo");
        // NuevaCarpeta/Ejemplo/Ejemplo
        System.out.println(unDir.resolve(unFile.getFileName()));

        // Relativizar una ruta respecto a otra
        Path otroFile = Path.of("NuevaCarpeta/Ejemplo1");
        Path otroDir = Path.of("NuevaCarpeta");
        // obtener c/d.txt
        ...
        Files.createDirectories(Path.of("NuevaCarpeta/Ejemplo2"));

        // Imprimir el contenido de un directorio
        Files.list(Path.of("carpetaPrueba")).forEach(System.out::println);

        // Obtener la lista de los contenidos anidados de un directorio
        List<Path> list = Files.walk(Paths.get("ProgramacionApolinar/")).toList();

        // Imprimir los ficheros regulares de un directorio ordenados por tamaño
        Files.list(Path.of("ProgramacionApolinar/src"))
        ...
        // Recorre e Imprime los archivos ordenados
